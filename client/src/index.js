...
class BuySell extends React.Component {
  ...
  handleSell(event) {
    event.preventDefault();
    fetch("http://localhost:5500/sell", {
      headers: {
        "Content-type": "application/json",
      },
      credentials: "include",
...
class StockSearch extends React.Component {
  ...
  update() {
    const getBal = fetch(`//localhost:5500/getBal`, { credentials: "include" })
      .then((response) => response.json())
      .then((data) => data.bal)
      .catch((err) => {
        console.err(err);
      });
    const getStocks = fetch(`//localhost:5500/getStocks`, {
      credentials: "include",
    })
      .then((response) => response.json())
  ...
  handleSubmit(event) {
    ...
    fetch(`//localhost:5500/getQuote/${stock}`)
      .then((response) => response.json())
      .then((data) => {
        if (data.error) {
